	...
	public class OpenAIArticleTitleService
	{
		...
		public async Task<string> GenerateArticleTitleAsync(string topic)
		{
			...
			var requestBody = new
			{
				...
				messages = new[]
				{
				new { role = "system", content = "Sen profesyonel bir makaleler için başlık oluşturucususun." },
				new {
					role = "user",
					content = $"'{topic}' makale anahtar kelimelerini referans alarak bu konuya uygun birbirinden farklı 3 adet makale başlığı önermeni istiyorum, "
				}
			},
				temperature = 0.7,
				max_tokens = 1100
